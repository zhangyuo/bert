...
"""
CPU RUN TIME(4核 13G 2.2GHZ): 5 h 6 min ------ global_step/sec = 0.6
GPU RUN TIME(NVIDIA Tesla P100 16G): 15 min ------ global_step/sec = 16
TPU RUN TIME（Num TPU Cores: 8, Num TPU Workers: 1, Num TPU Cores Per Worker: 8）

参考：https://blog.csdn.net/jiaowoshouzi/article/details/89388794/
...
# 解析tensorflow命令行参数，通过 “flag_name = FLAGS.flag_name” 的格式获取参数，如batch_size=FLAGS.batch_size.
FLAGS = flags.FLAGS
...
## Required parameters
flags.DEFINE_string(
    "data_dir", "/Users/zhangyu/Documents/PycharmProject/bert/data/MRPC/",
    "The input data dir. Should contain the .tsv files (or other data files) "
    "for the task.")

flags.DEFINE_string(
    "bert_config_file", "/Users/zhangyu/Documents/PycharmProject/bert/model/uncased_L-12_H-768_A-12/bert_config.json",
...
flags.DEFINE_string("vocab_file",
                    "/Users/zhangyu/Documents/PycharmProject/bert/model/uncased_L-12_H-768_A-12/vocab.txt",
                    "The vocabulary file that the BERT model was trained on.")

flags.DEFINE_string(
    "output_dir", "/Users/zhangyu/Documents/PycharmProject/bert/data/tmp/",
    "The output directory where the model checkpoints will be written.")

## Other parameters

flags.DEFINE_string(
    "init_checkpoint", "/Users/zhangyu/Documents/PycharmProject/bert/model/uncased_L-12_H-768_A-12/bert_model.ckpt",
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    # 调用modeling.BertModel得到BERT模型，然后使用它的get_pooled_output方法得到[CLS]最后一层的输出，这是一个768(默认参数下)的向量，
    # 然后就是常规的接一个全连接层得到logits，然后softmax得到概率，之后就可以根据真实的分类标签计算loss。
...
# 使用tf.app.run作为入口时，主函数至少需要一个参数，常见的写法就是用“_”作为参数，写成main(_)，防止报错。
def main(_):
    ...
    processors = {
        ...
        "mrpc": MrpcProcessor,
        "xnli": XnliProcessor,
        'selfsim': SimProcessor  # 添加自己的processor
    }
...
if __name__ == "__main__":
    # 如下含义是进行安全检查确保被标志的命令行参数都有显示定义，即强制要求显示定义的命令行参数
