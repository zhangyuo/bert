# 项目说明
fork in google github: https://github.com/google-research/bert.git

项目对原始版本进行调整，从tf1.0 升级至 tf2.0，并针对run_classfier分类器进行测试（使用GLUE数据集MRPC）。

项目在kaggle提供的服务器中进行测试：包括CPU、GPU、TPU版本：

* CPU RUN TIME(4核 13G 2.2GHZ): 5 h 6 min ------ global_step/sec = 0.6
* GPU RUN TIME(NVIDIA Tesla P100 16G): 15 min ------ global_step/sec = 16
* TPU RUN TIME（Num TPU Cores: 8, Num TPU Workers: 1, Num TPU Cores Per Worker: 8）----测试未成功。kaggle TPU服务需使用云端IO地址，
由于Google服务器在外网资源，未翻墙无法登录，故无法创建模型输出目录，模型文件输出地址无法保存。输入预训练模型和数据文件可通过kaggle数据源创建进行grpc路径配置。
因此整体训练过程未完成，但run_classfier过程相同，配置完成，训练过程将成功，且训练过程会更高。对TPU训练过程感兴趣的同学可进行外网配置IO路径，并进行训练。
...
由于测试过程采用MRPC数据集（英文），故采用英文基础版本的预训练模型：uncased_L-12_H-768_A-12
